...
/**
 * MagneticGrid — "Dot Image Reveal" (adapted from Originkit to JSX).
 *
 * A grid of dots that grow into circular windows onto an image as the cursor
 * approaches, revealing the picture through a magnetic ripple. Rendered on a
 * DPR-aware <canvas>. The host fills its parent, so the PARENT must have an
 * explicit width/height for the ResizeObserver + canvas to size correctly.
 *
 * `image` accepts a string or { src, fallback, srcSet, alt }. WebP is tried
 * first; on load/decode failure it falls back to `fallback` (e.g. a PNG).
 */
function imgSrc(image) {
  return typeof image === 'string' ? image : image?.src || '';
}
function imgFallback(image) {
  return typeof image === 'string' ? '' : image?.fallback || '';
}
...
export default function MagneticGrid({
  ...
  const fallback = imgFallback(image);
  ...
  useEffect(() => {
    ...
    if (src) {
      ...
      img.onerror = () => {
        // WebP unsupported / failed → drop to the raster fallback once.
    ...
    const drawFrame = (now) => {
      ...
      for (const cell of cells) {
        ...
        cell.draw(ctx, img, dotSize, fullSize, fitInfo, color);
      }
    };
  ...
  }, [src, fallback, dots, gap, intensity, radius, color]);

  return (
    <div
      ref={hostRef}
      style={{ position: 'relative', width: '100%', height: '100%', overflow: 'visible', background, ...(style || {}) }}
    >
